...
![image-20200914171545936](C:\Users\aprz512\AppData\Roaming\Typora\typora-user-images\image-20200914171545936.png)
...
Lcrack;->log 是调用一个叫 crack 类的 log 方法。在你想要输出log的位置直接粘上去就好了，因为我们调用的是静态方法，所以还不用考虑变量问题，是不是很简单呢：

![](18.png)

显然，我们需要往 apk 里面添加这个类（类文件已经提供好了）。过程如下：

先使用工具将 class.dex 文件反编译 ：

![](16.png)

然后再反编译出来的最外层添加 crack.smali 就好了，这里的apk是已经添加好了的，建议自己添加log试试：
...
可以右键添加自己的log，然后把文件也添加进去，里面的代码自己编辑，然后转到smali代码，右键插入代码即可搞定。
